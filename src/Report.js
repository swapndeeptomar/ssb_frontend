...
export default function Report({ onRestart, sessionId }) {
  ...
  const getDynamicTip = (feedback, score, testType) => {
  ...
  const downloadPDF = () => {
    ...
    const tableRows = data.details.map(item => {
        let tip = getDynamicTip(item.feedback, item.score, item.type);
        tip = tip.replace(/([\u2700-\u27BF]|[\uE000-\uF8FF]|\uD83C[\uDC00-\uDFFF]|\uD83D[\uDC00-\uDFFF]|[\u2011-\u26FF]|\uD83E[\uDD10-\uDDFF])/g, '');
    ...
    autoTable(doc, {
        ...
        bodyStyles: { fontSize: 9, cellPadding: 3, valign: 'top', overflow: 'linebreak' },
        columnStyles: {
            0: { cellWidth: 18, fontStyle: 'bold' },
            1: { cellWidth: 55, fontStyle: 'italic' },
    ...
    // --- TABLE 2: OLQ ASSESSMENT (NEW ADDITION) ---
    let finalY = doc.lastAutoTable.finalY + 15; // Pichi table ke niche gap do

    // Check karo agar page khatam hone wala hai
    if (finalY > 250) {
        doc.addPage();
        finalY = 20;
    }
        ...
        <table style={styles.table}>
          <thead>
            <tr style={{backgroundColor: '#ecf0f1'}}>
              ...
              <th style={{...styles.th, width: '30%'}}>Your Answer</th>
              <th style={{...styles.th, width: '50%'}}>AI Analysis & Feedback</th>
          ...
          <tbody>
            {data.details.map((item, index) => (
              <tr key={index} style={{borderBottom: '1px solid #eee'}}>
                ...
                <td style={{...styles.td, fontStyle: 'italic', color: '#555', fontSize: '14px'}}>"{item.answer}"</td>
                <td style={styles.td}>
                    ...
                    <div style={{backgroundColor: '#e8f4fc', padding: '8px', borderRadius: '5px', fontSize: '13px', color: '#2980b9', borderLeft: '3px solid #3498db'}}>
                        {"💡 " + getDynamicTip(item.feedback, item.score, item.type)}
                    </div>
                </td>
      ...
      {data.olq_analysis && (
        <div style={{marginTop: '30px', padding: '20px', backgroundColor: '#fff', border: '1px solid #ddd', borderRadius: '10px'}}>
            ...
            <div style={{marginTop: '20px', padding: '15px', backgroundColor: '#f0f9ff', borderRadius: '8px', borderLeft: '5px solid #3498db'}}>
                <strong>💪 Strong Areas:</strong> {data.strong_areas?.join(", ")}
                <br/>
...
const styles = {
  ...
  td: { padding: '15px', verticalAlign: 'top', wordWrap: 'break-word', overflowWrap: 'break-word' },
